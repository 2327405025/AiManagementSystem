...
这不是因为 PostgreSQL 在所有场景都比 MySQL 快；对于 10 万条任务，两者都能轻松承载。选择 PostgreSQL 的原因是它更贴合当前模型和后续演进：

1. **关系完整性优先**：任务依赖是带复合主键、双外键和检查约束的图关系。PostgreSQL 在复杂约束、事务语义和递归 CTE 方面成熟，适合后续把超大依赖树下推到数据库。
2. **时间语义清晰**：当前实体使用 Java `Instant`，迁移脚本使用 `TIMESTAMP WITH TIME ZONE`，可避免服务器时区变化造成歧义。
3. **稳定的深分页与诊断能力**：`(created_at, id)` 复合索引支持键集分页；成熟的执行计划、慢查询分析和会话级 `statement_timeout` 便于控制尾延迟。
4. **AI 数据演进空间**：虽然当前向量检索使用 ChromaDB，PostgreSQL 的全文检索、JSONB 和 pgvector 为未来减少组件数量提供了可选路径。
5. **避免多数据库双写**：任务、标签、依赖和幂等记录都保留在同一事务数据库中，Redis/Chroma 只作为可丢失后重建的派生数据。

MySQL 8 同样支持事务、外键、递归 CTE 和复合索引。如果团队的 MySQL 运维能力明显更强，选择 MySQL 完全合理，但需要调整 Flyway 方言、时间列、连接参数并重新验证查询计划；Spring Data JPA 的分层设计使这种迁移不影响 Controller 和大部分 Service 逻辑。
